...
public class BibTeX2DTM
{
	...
	private String[] fieldsToImportDefault = { "title", "keywords", "abstract" };

	private String[] fieldsToImportAlternative = {  };
	...
	public void write() throws IOException
	{
		...
		// Dump .bib
		FileWriter fw = new FileWriter(basedir + File.separator + corpusName + BIBTEX_EXTENSION);
		FieldFormatter ff = new LatexFieldFormatter();
		iDocs = entries.iterator();
		while (iDocs.hasNext()) {
			BibtexEntry entry = iDocs.next();
			entry.write(fw, ff, true);
			fw.write("\n");
		}
	...
	public void read() throws IOException
	{
		...
		TextPipelinePreprocessor alternativePreprocessor = new AlternativeTextPipelinePreprocessor();
		TextPipelinePreprocessor defaultPreprocessor = new DefaultTextPipelinePreprocessor();

		alternativePreprocessor.setUseStemmer(useStemmer);
		alternativePreprocessor.setUseStopwords(useStopwords);
		alternativePreprocessor.setLUCut(useLUCut);

		defaultPreprocessor.setUseStemmer(useStemmer);
		defaultPreprocessor.setUseStopwords(useStopwords);
		defaultPreprocessor.setLUCut(useLUCut);
		...
		while (iterator.hasNext()) {
			...
			for (String field : fieldsToImportDefault) {
			...
			*/
			if (sb.length() > 0) {
				parser.setReader(new StringReader(sb.toString()));
				parser.setConsumer(defaultPreprocessor.getStart());
				defaultPreprocessor.getEnd().setConsumer(documentProcessor);
				parser.start();
				parser.stop();
			}
				
			sb = new StringBuilder();
			for (String field : fieldsToImportAlternative) {
				String value = entry.getField(field);
				if (value != null && ! value.trim().isEmpty()) {
					sb.append(value);
					sb.append(" ");
			...
			*/
			if (sb.length() > 0) {
				parser.setReader(new StringReader(sb.toString()));
				parser.setConsumer(alternativePreprocessor.getStart());
				alternativePreprocessor.getEnd().setConsumer(documentProcessor);
				parser.start();
				parser.stop();
			}
		}
