...
import lode.miner.analysis.BagOfWordsBuilder;
import lode.miner.analysis.FrequencyResourcePowerCalculator;
import lode.miner.analysis.LUCut4PostingList;
import lode.miner.analysis.PostingLists2Resource;
import lode.miner.analysis.PostingListsBuilder;
import lode.miner.analysis.PowerCalculatorComponent;
import lode.miner.analysis.ResourcePowerCalculator;
...
{  
	...
	public void setupPipeline()
    {
		...
		BagOfWordsBuilder termBagBuilder = new BagOfWordsBuilder();
		PostingListsBuilder termsPostingListsBuilder = new PostingListsBuilder();
		PowerCalculatorComponent termsFrequencyPowerCalculator = new PowerCalculatorComponent();
		ResourcePowerCalculator frequencyPowerCalculator = new FrequencyResourcePowerCalculator();
		LUCut4PostingList lucut4postings = new LUCut4PostingList();
		PostingLists2Resource postConverter = new PostingLists2Resource();
		...
		*/
    	...
    	stemmerDecision.setConsumerFalse(termBagBuilder);
    	
		stemmerTransformer.setStemmer(stemmer);
		stemmerTransformer.setConsumer(termBagBuilder);
				
		termBagBuilder.setConsumer(termsPostingListsBuilder);
		termsPostingListsBuilder.setConsumer(termsFrequencyPowerCalculator);
		termsFrequencyPowerCalculator.setCalculator(frequencyPowerCalculator);
		termsFrequencyPowerCalculator.setConsumer(lucut4postings);
		lucut4postings.setMin(1);
		lucut4postings.setMax(10000);
		lucut4postings.setConsumer(postConverter);
		
		postConverter.setConsumer(bufferComponent);
